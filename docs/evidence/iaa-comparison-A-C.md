...
| D16_b17 | FAM/implicit/self `딸` | FAM/inferential/self `딸 야자` | ① 경계 · ② 등급 |
| D20_b18 | AGE/explicit/self `쉰둘인데` | AGE/explicit/self `쉰둘인데` | 완전 일치 |
| E08_b14 | FAM/implicit/self `도련님` | — | ③ 누락 (C가 못 찾음) |
| E21_b08 | FAM/implicit/self `할멈` · LOC_ADMIN/explicit/self `남강` | — | ③ 누락 (C가 못 찾음) |

## 패턴 요약

**② 등급 — 가장 빈번.** A19_b16·C15_b15·C17_b09·D16_b17에서 A가
`implicit`을 쓰는 자리에 C는 일관되게 `inferential`을 씁니다.
label-schema §4-1 기준("이 스팬 하나만으로 속성이 거의 확정되는가")
으로 재점검이 필요합니다.

**① 경계 — 조사·어미.** B04_b02·D12_b04·D20_b18에서 C가 조사를
...
(C가 완전히 놓침)처럼, 규칙의 문제가 아니라 각자 읽으며 놓친
경우입니다. 합의 대상이 아니라 그냥 확인 사항입니다.

## 완전 일치

A05_b09, D07_b16(둘 다 스팬 0건), D20_b18(스팬 내용까지 완전 일치) —
셋은 갈림 없이 그대로 일치합니다.
