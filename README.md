...
각 역할이 자기 파트를 맡아 **목요일까지 초안, 금요일 최종**으로 낸다 (수→목, [#210](../../issues/210) — 수~목 검수와 겹치지 않게).
...
### 목 — 설계서 데이터 파트 초안 (수→목, #210) · 금 — 최종 + `w04` 태그 지원
...
- [ ] **검수 100~125 스팬** → `gold/<pid>_spans.jsonl` (수~목) ⭐
- [ ] 코퍼스 v1 동결 선언 (audit + validate 통과)
- [x] ~~D17·B16 처리~~ ✅ #201 전량 재생성
- [ ] created_at 시각(active_windows) 결정 — 날짜는 #201 로 해소
- [ ] 파인튜닝 train/test 분리 완료
- [ ] 설계서 데이터 파트 초안(목) · 최종(금)
...
- [ ] ~~행정구역 계층 픽스처~~ → D 대행 결과 검토
- [ ] ~~특정성 L1 fixture 18/18~~ → D 대행 결과 검토
- [ ] 설계서 특정성 파트 — D 초안 검토 30분 (목)
...
- [ ] D 인물 24명 검수 100~125 스팬 (수~목)
- [ ] C 대행: 지명 사전 계층 #115 · L1 픽스처 18/18 · 설계서 특정성 초안 (#210)
- [ ] 교사 라벨 8샤드 완료 → #204·#210 알림
- [ ] 멘토링 질문 3건 (HOLD·IAA·v0 라벨) ⭐
- [ ] 설계서 취합·제출 (금)
- [ ] `w04` 태그
...
### 수~목 — E 인물 21명 검수 100~125 스팬 ([#210](../../issues/210)) · 목 — 설계서 시스템 파트 초안 (수→목)
...
- [ ] E 인물 21명 검수 100~125 스팬 (수~목) — 교사 라벨은 9/9 아침 `gold/detect/` 에 있다
- [ ] `test_export.py` 통과
- [ ] 설계서 시스템 파트 완성
- [ ] AWS 크레딧 조건 확인 (금)
...
| 화 9/8 | | A·C | ~~IAA 파일럿~~ ✅ 22.9% (9/7) → #200 조정 |
