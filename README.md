...
[위 킥오프 §①](#-test-set-구성-방향-w4-이월--담당-a--pm) 표 참고:

| 선택지 | 내용 | 비고 |
|---|---|---|
| **(가)** | `gold/blind/` 스팬을 test로 재활용 | 빠르지만 blind 조건 논란 가능 |
| **(나)** | 단서 0 글에서 새로 뽑아 A가 라벨 | 깔끔하지만 추가 시간 필요 |
| **(다)** | `gold/detect/` 교사 라벨 그대로 | 빠르지만 라벨 품질 낮음 |

결정 후 즉시 이슈(#235) 에 선택지와 이유 한 줄 댓글 → B에게 알린다.
