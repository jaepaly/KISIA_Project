...
|---|---|---|
| 인물 | 35명 | **115명** ✅ |
| 글 (커밋) | 0편 | **3,094편** ✅ (E15 3편 미커밋 포함 시 3,097편) |
| 코퍼스 결함 | — | clue null 버그 수정 · 2,792편 패치 ([#189](../../pull/189), 머지 대기) |
...
| blind 라벨링 | — | 159편 배정 · jhyun114 110스팬 라벨링 완료 ([#191](../../pull/191) Draft — 머지 대기) |
| IAA | — | 16편 배정 ([#192](../../pull/192) — 머지 대기) · 실측 W4 이월 |
...
| 방향 | 내용 | 비용 |
|---|---|---|
| **(가) generated_at 역산** | `datetime(2026,3,2)` 리터럴을 제거하고 `generated_at` 기준으로 역산 | 스크립트 1개 + 전체 posts 덮어쓰기 |
| **(나) 수용** | `created_at` 을 파인튜닝 입력에서 제외하고 코퍼스 메타에 주석 | 스크립트 없음 |

**파인튜닝 입력에 `created_at` 을 쓸 계획이 없으면 (나) 가 맞다.** A·B 가 결정.

#### ③ D17·B16 재생성 범위 ([#184](../../issues/184)) — 담당 A

```
D17: b19~b30 = b01~b12 재탕 (간격 18, resume 오류)  → 12편 재생성 필요
B16: 「모니터암 교체」 b09·b19·b30 3회 중복           → 2편 재생성 필요
```

**코퍼스 v1 동결 전에 결정**한다. 재생성 시 blind/IAA _assignment 에 해당 글이 있으면 재배정 필요.

#### ④ cross-persona 중복 · S14 미달 처리 ([#183](../../issues/183), [#177](../../issues/177)) — 전원

```
#183: 동일 도입부 문장 40건 cross-persona — 측정 지표 보정 vs 재생성 vs 수용
#177: S14 카드 문장 길이 -32~-37% 미달 인물 4명 — 동결 전에 판단
```

**이 두 건이 동결 선행조건**이다. 수용 또는 처리 결정을 수요일 동결 선언 전에 확정.
...
| PR [#191](../../pull/191) Draft → Ready 전환 | C(jhyun114) | **월** |
| PR [#189](../../pull/189) 머지 | nuewsun (리뷰 후) | — |
| PR [#192](../../pull/192) 머지 | nuewsun (approved) | — |
| PR [#180](../../pull/180) 머지 | nuewsun (리뷰 후) | — |
| PR [#181](../../pull/181) CHANGES_REQUESTED 수정 | C(jhyun114) | **수** |
| created_at 처리 ([#182](../../issues/182)) | A (결정 후) | **수** |
| D17·B16 재생성 ([#184](../../issues/184)) | A (결정 후) | **수** |
| cross-persona 중복 처리 방향 ([#183](../../issues/183)) | 전원 결정 | **월** |
...
현재 [목표] 지표 두 가지 — M1 실측으로 채운다
  ...
  · Qwen3 vs 외부 LLM 속성 적중 일치율 ≥ 0.8 → exp04 결과 필요 (현재 실행기만 있고 결과 없음)
    ⚠️ exp05는 1.7B vs 4B 비교이므로 이 지표의 근거로 쓸 수 없다 — W4 중 exp04 실행 필요
```
...
### 월 오후 — PR #191 머지 확인 + 골드셋 현황 정리

jhyun114 의 PR [#191](../../pull/191) 이 Draft 상태다. Ready 로 전환하도록 요청 후 머지되면 `data/corpus/v0/gold/blind/` 스팬 파일들이 들어온다. 이 파일들이 IAA 파일럿 입력이다.
...
1. **D17·B16 재생성** (킥오프 결정에 따라): `generate.py --resume` 으로 해당 post_id 만 재실행
...
| 월 | 오후 | C | PR #181 수정 재제출 + PR #191 Ready 전환 |
...
**PR — nuewsun 리뷰 대기**
- [ ] [#189](../../pull/189) clue null 버그 수정 + E 인물 150편
- [ ] [#192](../../pull/192) sample_gold.py 수정 — approved, 머지만 남음
- [ ] [#180](../../pull/180) 문장 분리기 자모 이모티콘 수정

**PR — 당사자 수정 필요**
- [ ] [#191](../../pull/191) jhyun114 blind 110스팬 — Draft → Ready 전환 (C)
