...
| IAA | — | 1차 16편 실측 **22.9%** (9/7, [#188](../../pull/188) ✅) → [#200](../../issues/200) 기준 조정 → 새 표본 16편으로 2차 (재생성으로 옛 표본 무효) |
...
| 골드셋 검수 | — | ⚠️ **0건** — 입력인 교사 라벨(`label.py` 탐지본)이 코퍼스 전량에 돌아간 적 없음 ([#204](../../issues/204)) → A 실행(급하면 PM 대행) → 그 뒤 B·D·E 100~125 |
...
⚠️ 9/8 확인 — **검수한 스팬은 0건**이다. 검수의 입력인 교사 라벨(④ `label.py` 탐지본)이 코퍼스 전량에 돌아간 적이 없다 ([#204](../../issues/204)). 순서: **A 가 `label.py` 실행(급하면 PM 이 claude CLI 로 대행, [#205](../../pull/205)) → `gold/detect/<pid>_spans.jsonl` → B·D·E 각자 인물 100~125 스팬 검수.** C 의 blind 200 재라벨은 독립이며 교사 출력을 보기 전에 한다.
...
| ~~**IAA 파일럿** (A·C 16편)~~ | ✅ 22.9% (9/7) → [#200](../../issues/200) 기준 조정 후 새 표본 2차 | — |
...
| **교사 라벨 `label.py` 전량 실행** ([#204](../../issues/204)) — B·D·E 검수의 게이트 | **A** (급하면 PM 대행) | **수** |
...
### ~~화 — IAA 파일럿 라벨링~~ ✅ 9/7 실측 22.9% → [#200](../../issues/200) 기준 조정이 남은 일

### 화~수 — 교사 라벨 전량 실행 ⭐ ([#204](../../issues/204))

B·D·E 검수의 게이트다. 코퍼스 전량에 한 번도 돌아간 적이 없다.

```bash
# 탐지본 (기본). 교사는 생성(gpt)과 다른 계열 — Claude
python -X utf8 -m kopl.c5_corpus.label --posts data/corpus/v0/posts --out data/corpus/v0/gold \n  --provider anthropic --model claude-sonnet-4-6
# API 키가 없으면 CLI 경로 (#205): --provider cli --cli-cmd "claude -p --model claude-sonnet-4-6" --model claude-sonnet-4-6
```

출력 `gold/detect/<pid>_spans.jsonl` 115개. 급하면 PM 이 대행한다 — 이슈에서 정한다.
...
2. **1단 파인튜닝 착수** (W4 신규) — ⚠️ 학습 입력인 검수분이 아직 0건. 선행: A 교사 라벨([#204](../../issues/204)) → B·D·E 검수 → 그 뒤 BIO 변환
...
- [x] ~~**중단 기준 판정 기록**~~ ✅ HOLD
- [ ] #202 에 Gemini 출력 파일 추가 → 머지 · #201 뒤 재측정
- [ ] B 인물 21명 검수 100~125 스팬 (교사 라벨 뒤)
- [ ] 파인튜닝 학습 잡 최소 1회 성공
- [ ] 설계서 1단 파트 완성
...
### ~~화 — IAA 파일럿 라벨링~~ ✅ 9/7 실측 22.9% (partial) · exact 0%

갈린 자리 넷(경계·수준·누락·유형)은 [#200](../../issues/200) 에서 A 와 기준을 맞춘 뒤, **새 표본 16편**으로 2차 — 옛 표본은 재생성으로 무효.

### 화~ — blind 200 재라벨 ⭐ ([#201](../../pull/201) 합의)

9/8 코퍼스 전량 재생성으로 blind 배정 글의 본문이 전부 바뀌었다. `gold/blind/_assignment.json` 배정은 그대로, **새 본문으로 다시 매긴다.** 조건은 같다 — 교사 출력(`gold/detect/`)을 보기 전에. 그래서 순서상 A 의 교사 라벨보다 **먼저** 끝내는 게 맞다.
...
- [x] ~~IAA 1차 16편 · 등급별 F1~~ ✅ 22.9% (9/7)
- [ ] [#200](../../issues/200) 기준 합의 → 새 표본 2차 IAA
- [ ] **blind 200 재라벨** (새 본문 · 교사 출력 보기 전) ⭐
...
- [ ] D 인물 24명 검수 100~125 스팬 (교사 라벨 뒤 · [#204](../../issues/204))
...
- [ ] E 인물 21명 검수 100~125 스팬 — **교사 라벨([#204](../../issues/204))이 돌아간 뒤.** 그 전엔 할 게 없다 (E-system.md W3 「골드셋 검수」가 이월된 것)
- [ ] `test_export.py` 통과
- [ ] 설계서 시스템 파트 완성
- [ ] AWS 크레딧 조건 확인 (금)
