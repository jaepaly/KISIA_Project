...
- 첫 화면(`/`)에 **첫 방문 모달** + 「▶ 3분 체험 시작」 → **11단계 투어**가 시연 순서대로 데려간다 (마당일기 → 내 글 점검 → 421 →
  깔때기 → 위치태그 끄기 → 421→111,069 → 글쓰기 → 예문·체험 계정·점검 → 색칠된 표현 → 작성자별 비교 → 사람마다 다른 숫자). 발표 대본이 곧 이 11단계.
...
1. **배포** (AWS 오면 즉시) — 한 인스턴스, `PADOPOOL_URL`·`SNS_URL`·`DEMO_EXTERNAL_REWRITE=true`·`ANTHROPIC_API_KEY`.
   `data/interim/sns.db` 는 배포 뒤 `seed.py --reset` 으로 만든다. 배포 링크로 투어 11단계를 한 번 끝까지 돌려 본다 (휴대폰도).
