...
# 방문지·이사는 양쪽에 걸친다 (내 방문/남의 방문 · 남의 이사/내 이사 계획 — §4-4-1)
SELF_VIA = ("출신지", "과거거주", "과거근무지", "방문지", "이사")
BOTH_VIA = ("방문지", "이사")
# 통로 목록 — 새 값을 쓰려면 §4-4-1 표에 먼저 추가한다
TRAP_VIA = ("거주", "근무지", "이직·발령", "개업", "이사", "거래·납품",
            "행사·모임", "방문지", "출신지", "과거거주", "과거근무지")
...
def validate(persona: dict) -> list[Issue]:
    ...
    for i, c in enumerate(clue_plan):
        ...
        # subject 와 통로가 맞는가. 방문지·이사는 양쪽에 걸친다 (§4-4-1)
        if via in SELF_VIA and via not in BOTH_VIA and subj != "self":
            warn(f"clue_plan[{i}]",
