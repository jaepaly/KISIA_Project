...
#
# ⚠️ **자모 이모티콘도 문장 끝이다.** 「신났음ㅠ」 「착 내려감ㅠ」 「나갔음ㅋㅋ」 처럼
# 자모가 음절에 바로 붙으면 뒤의 \s+ 가 안 맞아 앞뒤가 한 문장으로 붙는다. 코퍼스에
# 674자리 있고 S18(입시일기)·S17 인물에 몰린다 — A08 의 문장길이가 41.05자로
# 재어져 선언 18자 대비 +128% 로 보였다. 자모를 경계로 넣으면 24.06자다.
# ⚠️ 종결어미 목록으로는 못 잡는다. 「놓침」 「내려감」 같은 명사형 종결의 받침 ㅁ 을
# 목록에 넣으면 「지금」(2,462) 「사람」(356) 「처럼」(545) 까지 끊긴다 — 목록 밖
# ㅁ받침이 11,691자리다. 그래서 어미가 아니라 **자모** 를 신호로 쓴다.
SENT_SPLIT = re.compile(
    r"(?:(?<!\.)\.(?!\.)|[!?]+|"                    # 홑마침표만. .. 과 ... 은 뺀다
    r"(?<=[요음함다죠네])[.…]*\s+|"                    # 종결어미 (뒤에 말줄임표가 붙어도)
    r"(?<=[요음함다죠네])[.…]{2,}|"                    # 종결어미 + 말줄임표 (공백 없이 이어져도)
    r"(?<=[가-힣])[ㅋㅎㅜㅠㅡㅅ]+[.…]*(?:\s+|$))"      # 음절에 붙은 자모 이모티콘
)
